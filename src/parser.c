/* Automatically generated by tree-sitter v0.25.3 (2a835ee029dca1c325e6f1c01dbce40396f6123e) */

#include "tree_sitter/parser.h"

#if defined(__GNUC__) || defined(__clang__)
#pragma GCC diagnostic ignored "-Wmissing-field-initializers"
#endif

#define LANGUAGE_VERSION 15
#define STATE_COUNT 93
#define LARGE_STATE_COUNT 2
#define SYMBOL_COUNT 78
#define ALIAS_COUNT 0
#define TOKEN_COUNT 45
#define EXTERNAL_TOKEN_COUNT 0
#define FIELD_COUNT 6
#define MAX_ALIAS_SEQUENCE_LENGTH 6
#define MAX_RESERVED_WORD_SET_SIZE 0
#define PRODUCTION_ID_COUNT 4
#define SUPERTYPE_COUNT 0

enum ts_symbol_identifiers {
  anon_sym_func = 1,
  anon_sym_DASH_GT = 2,
  sym_identifier = 3,
  anon_sym_LPAREN = 4,
  anon_sym_COMMA = 5,
  anon_sym_RPAREN = 6,
  anon_sym_u8 = 7,
  anon_sym_i8 = 8,
  anon_sym_u16 = 9,
  anon_sym_i16 = 10,
  anon_sym_u32 = 11,
  anon_sym_i32 = 12,
  anon_sym_u64 = 13,
  anon_sym_i64 = 14,
  anon_sym_f32 = 15,
  anon_sym_f64 = 16,
  anon_sym_LBRACE = 17,
  anon_sym_RBRACE = 18,
  sym_oneline_comment = 19,
  sym_multiline_comment = 20,
  anon_sym_SEMI = 21,
  anon_sym_LT = 22,
  anon_sym_GT = 23,
  anon_sym_LT_EQ = 24,
  anon_sym_GT_EQ = 25,
  anon_sym_EQ_EQ = 26,
  anon_sym_EQ = 27,
  anon_sym_PLUS = 28,
  anon_sym_DASH = 29,
  anon_sym_STAR = 30,
  anon_sym_SLASH = 31,
  anon_sym_PERCENT = 32,
  sym_integer_literal = 33,
  sym_floating_point_literal = 34,
  sym_string_literal = 35,
  anon_sym_var = 36,
  anon_sym_let = 37,
  anon_sym_COLON = 38,
  anon_sym_return = 39,
  anon_sym_break = 40,
  anon_sym_loop = 41,
  anon_sym_if = 42,
  anon_sym_elif = 43,
  anon_sym_else = 44,
  sym_source_file = 45,
  sym__definition = 46,
  sym__declaration = 47,
  sym_function_definition = 48,
  sym_parameter_list = 49,
  sym__type = 50,
  sym_builtin_type = 51,
  sym_user_type = 52,
  sym_block = 53,
  sym__sentence = 54,
  sym_expression_sentence = 55,
  sym_variable_declaration_sentence = 56,
  sym_variable_definition_sentence = 57,
  sym__callable_expression = 58,
  sym__uncallable_expression = 59,
  sym__expression = 60,
  sym_function_call = 61,
  sym_binary_expresion = 62,
  sym__literal = 63,
  sym_variable_reference_expression = 64,
  sym_variable_declaration = 65,
  sym_variable_definition = 66,
  sym_return_sentence = 67,
  sym_break_sentence = 68,
  sym_loop_statement = 69,
  sym_if_statement = 70,
  sym_elif_statement = 71,
  sym_else_statement = 72,
  sym__blocky = 73,
  aux_sym_source_file_repeat1 = 74,
  aux_sym_parameter_list_repeat1 = 75,
  aux_sym_block_repeat1 = 76,
  aux_sym_function_call_repeat1 = 77,
};

static const char * const ts_symbol_names[] = {
  [ts_builtin_sym_end] = "end",
  [anon_sym_func] = "func",
  [anon_sym_DASH_GT] = "->",
  [sym_identifier] = "identifier",
  [anon_sym_LPAREN] = "(",
  [anon_sym_COMMA] = ",",
  [anon_sym_RPAREN] = ")",
  [anon_sym_u8] = "u8",
  [anon_sym_i8] = "i8",
  [anon_sym_u16] = "u16",
  [anon_sym_i16] = "i16",
  [anon_sym_u32] = "u32",
  [anon_sym_i32] = "i32",
  [anon_sym_u64] = "u64",
  [anon_sym_i64] = "i64",
  [anon_sym_f32] = "f32",
  [anon_sym_f64] = "f64",
  [anon_sym_LBRACE] = "{",
  [anon_sym_RBRACE] = "}",
  [sym_oneline_comment] = "oneline_comment",
  [sym_multiline_comment] = "multiline_comment",
  [anon_sym_SEMI] = ";",
  [anon_sym_LT] = "<",
  [anon_sym_GT] = ">",
  [anon_sym_LT_EQ] = "<=",
  [anon_sym_GT_EQ] = ">=",
  [anon_sym_EQ_EQ] = "==",
  [anon_sym_EQ] = "=",
  [anon_sym_PLUS] = "+",
  [anon_sym_DASH] = "-",
  [anon_sym_STAR] = "*",
  [anon_sym_SLASH] = "/",
  [anon_sym_PERCENT] = "%",
  [sym_integer_literal] = "integer_literal",
  [sym_floating_point_literal] = "floating_point_literal",
  [sym_string_literal] = "string_literal",
  [anon_sym_var] = "var",
  [anon_sym_let] = "let",
  [anon_sym_COLON] = ":",
  [anon_sym_return] = "return",
  [anon_sym_break] = "break",
  [anon_sym_loop] = "loop",
  [anon_sym_if] = "if",
  [anon_sym_elif] = "elif",
  [anon_sym_else] = "else",
  [sym_source_file] = "source_file",
  [sym__definition] = "_definition",
  [sym__declaration] = "_declaration",
  [sym_function_definition] = "function_definition",
  [sym_parameter_list] = "parameter_list",
  [sym__type] = "_type",
  [sym_builtin_type] = "builtin_type",
  [sym_user_type] = "user_type",
  [sym_block] = "block",
  [sym__sentence] = "_sentence",
  [sym_expression_sentence] = "expression_sentence",
  [sym_variable_declaration_sentence] = "variable_declaration_sentence",
  [sym_variable_definition_sentence] = "variable_definition_sentence",
  [sym__callable_expression] = "_callable_expression",
  [sym__uncallable_expression] = "_uncallable_expression",
  [sym__expression] = "_expression",
  [sym_function_call] = "function_call",
  [sym_binary_expresion] = "binary_expresion",
  [sym__literal] = "_literal",
  [sym_variable_reference_expression] = "variable_reference_expression",
  [sym_variable_declaration] = "variable_declaration",
  [sym_variable_definition] = "variable_definition",
  [sym_return_sentence] = "return_sentence",
  [sym_break_sentence] = "break_sentence",
  [sym_loop_statement] = "loop_statement",
  [sym_if_statement] = "if_statement",
  [sym_elif_statement] = "elif_statement",
  [sym_else_statement] = "else_statement",
  [sym__blocky] = "_blocky",
  [aux_sym_source_file_repeat1] = "source_file_repeat1",
  [aux_sym_parameter_list_repeat1] = "parameter_list_repeat1",
  [aux_sym_block_repeat1] = "block_repeat1",
  [aux_sym_function_call_repeat1] = "function_call_repeat1",
};

static const TSSymbol ts_symbol_map[] = {
  [ts_builtin_sym_end] = ts_builtin_sym_end,
  [anon_sym_func] = anon_sym_func,
  [anon_sym_DASH_GT] = anon_sym_DASH_GT,
  [sym_identifier] = sym_identifier,
  [anon_sym_LPAREN] = anon_sym_LPAREN,
  [anon_sym_COMMA] = anon_sym_COMMA,
  [anon_sym_RPAREN] = anon_sym_RPAREN,
  [anon_sym_u8] = anon_sym_u8,
  [anon_sym_i8] = anon_sym_i8,
  [anon_sym_u16] = anon_sym_u16,
  [anon_sym_i16] = anon_sym_i16,
  [anon_sym_u32] = anon_sym_u32,
  [anon_sym_i32] = anon_sym_i32,
  [anon_sym_u64] = anon_sym_u64,
  [anon_sym_i64] = anon_sym_i64,
  [anon_sym_f32] = anon_sym_f32,
  [anon_sym_f64] = anon_sym_f64,
  [anon_sym_LBRACE] = anon_sym_LBRACE,
  [anon_sym_RBRACE] = anon_sym_RBRACE,
  [sym_oneline_comment] = sym_oneline_comment,
  [sym_multiline_comment] = sym_multiline_comment,
  [anon_sym_SEMI] = anon_sym_SEMI,
  [anon_sym_LT] = anon_sym_LT,
  [anon_sym_GT] = anon_sym_GT,
  [anon_sym_LT_EQ] = anon_sym_LT_EQ,
  [anon_sym_GT_EQ] = anon_sym_GT_EQ,
  [anon_sym_EQ_EQ] = anon_sym_EQ_EQ,
  [anon_sym_EQ] = anon_sym_EQ,
  [anon_sym_PLUS] = anon_sym_PLUS,
  [anon_sym_DASH] = anon_sym_DASH,
  [anon_sym_STAR] = anon_sym_STAR,
  [anon_sym_SLASH] = anon_sym_SLASH,
  [anon_sym_PERCENT] = anon_sym_PERCENT,
  [sym_integer_literal] = sym_integer_literal,
  [sym_floating_point_literal] = sym_floating_point_literal,
  [sym_string_literal] = sym_string_literal,
  [anon_sym_var] = anon_sym_var,
  [anon_sym_let] = anon_sym_let,
  [anon_sym_COLON] = anon_sym_COLON,
  [anon_sym_return] = anon_sym_return,
  [anon_sym_break] = anon_sym_break,
  [anon_sym_loop] = anon_sym_loop,
  [anon_sym_if] = anon_sym_if,
  [anon_sym_elif] = anon_sym_elif,
  [anon_sym_else] = anon_sym_else,
  [sym_source_file] = sym_source_file,
  [sym__definition] = sym__definition,
  [sym__declaration] = sym__declaration,
  [sym_function_definition] = sym_function_definition,
  [sym_parameter_list] = sym_parameter_list,
  [sym__type] = sym__type,
  [sym_builtin_type] = sym_builtin_type,
  [sym_user_type] = sym_user_type,
  [sym_block] = sym_block,
  [sym__sentence] = sym__sentence,
  [sym_expression_sentence] = sym_expression_sentence,
  [sym_variable_declaration_sentence] = sym_variable_declaration_sentence,
  [sym_variable_definition_sentence] = sym_variable_definition_sentence,
  [sym__callable_expression] = sym__callable_expression,
  [sym__uncallable_expression] = sym__uncallable_expression,
  [sym__expression] = sym__expression,
  [sym_function_call] = sym_function_call,
  [sym_binary_expresion] = sym_binary_expresion,
  [sym__literal] = sym__literal,
  [sym_variable_reference_expression] = sym_variable_reference_expression,
  [sym_variable_declaration] = sym_variable_declaration,
  [sym_variable_definition] = sym_variable_definition,
  [sym_return_sentence] = sym_return_sentence,
  [sym_break_sentence] = sym_break_sentence,
  [sym_loop_statement] = sym_loop_statement,
  [sym_if_statement] = sym_if_statement,
  [sym_elif_statement] = sym_elif_statement,
  [sym_else_statement] = sym_else_statement,
  [sym__blocky] = sym__blocky,
  [aux_sym_source_file_repeat1] = aux_sym_source_file_repeat1,
  [aux_sym_parameter_list_repeat1] = aux_sym_parameter_list_repeat1,
  [aux_sym_block_repeat1] = aux_sym_block_repeat1,
  [aux_sym_function_call_repeat1] = aux_sym_function_call_repeat1,
};

static const TSSymbolMetadata ts_symbol_metadata[] = {
  [ts_builtin_sym_end] = {
    .visible = false,
    .named = true,
  },
  [anon_sym_func] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_DASH_GT] = {
    .visible = true,
    .named = false,
  },
  [sym_identifier] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_LPAREN] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_COMMA] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_RPAREN] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_u8] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_i8] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_u16] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_i16] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_u32] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_i32] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_u64] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_i64] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_f32] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_f64] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_LBRACE] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_RBRACE] = {
    .visible = true,
    .named = false,
  },
  [sym_oneline_comment] = {
    .visible = true,
    .named = true,
  },
  [sym_multiline_comment] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_SEMI] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_LT] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_GT] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_LT_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_GT_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_EQ_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_EQ] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_PLUS] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_DASH] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_STAR] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_SLASH] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_PERCENT] = {
    .visible = true,
    .named = false,
  },
  [sym_integer_literal] = {
    .visible = true,
    .named = true,
  },
  [sym_floating_point_literal] = {
    .visible = true,
    .named = true,
  },
  [sym_string_literal] = {
    .visible = true,
    .named = true,
  },
  [anon_sym_var] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_let] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_COLON] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_return] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_break] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_loop] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_if] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_elif] = {
    .visible = true,
    .named = false,
  },
  [anon_sym_else] = {
    .visible = true,
    .named = false,
  },
  [sym_source_file] = {
    .visible = true,
    .named = true,
  },
  [sym__definition] = {
    .visible = false,
    .named = true,
  },
  [sym__declaration] = {
    .visible = false,
    .named = true,
  },
  [sym_function_definition] = {
    .visible = true,
    .named = true,
  },
  [sym_parameter_list] = {
    .visible = true,
    .named = true,
  },
  [sym__type] = {
    .visible = false,
    .named = true,
  },
  [sym_builtin_type] = {
    .visible = true,
    .named = true,
  },
  [sym_user_type] = {
    .visible = true,
    .named = true,
  },
  [sym_block] = {
    .visible = true,
    .named = true,
  },
  [sym__sentence] = {
    .visible = false,
    .named = true,
  },
  [sym_expression_sentence] = {
    .visible = true,
    .named = true,
  },
  [sym_variable_declaration_sentence] = {
    .visible = true,
    .named = true,
  },
  [sym_variable_definition_sentence] = {
    .visible = true,
    .named = true,
  },
  [sym__callable_expression] = {
    .visible = false,
    .named = true,
  },
  [sym__uncallable_expression] = {
    .visible = false,
    .named = true,
  },
  [sym__expression] = {
    .visible = false,
    .named = true,
  },
  [sym_function_call] = {
    .visible = true,
    .named = true,
  },
  [sym_binary_expresion] = {
    .visible = true,
    .named = true,
  },
  [sym__literal] = {
    .visible = false,
    .named = true,
  },
  [sym_variable_reference_expression] = {
    .visible = true,
    .named = true,
  },
  [sym_variable_declaration] = {
    .visible = true,
    .named = true,
  },
  [sym_variable_definition] = {
    .visible = true,
    .named = true,
  },
  [sym_return_sentence] = {
    .visible = true,
    .named = true,
  },
  [sym_break_sentence] = {
    .visible = true,
    .named = true,
  },
  [sym_loop_statement] = {
    .visible = true,
    .named = true,
  },
  [sym_if_statement] = {
    .visible = true,
    .named = true,
  },
  [sym_elif_statement] = {
    .visible = true,
    .named = true,
  },
  [sym_else_statement] = {
    .visible = true,
    .named = true,
  },
  [sym__blocky] = {
    .visible = false,
    .named = true,
  },
  [aux_sym_source_file_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_parameter_list_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_block_repeat1] = {
    .visible = false,
    .named = false,
  },
  [aux_sym_function_call_repeat1] = {
    .visible = false,
    .named = false,
  },
};

enum ts_field_identifiers {
  field_body = 1,
  field_callee = 2,
  field_name = 3,
  field_parameters = 4,
  field_return_type = 5,
  field_type = 6,
};

static const char * const ts_field_names[] = {
  [0] = NULL,
  [field_body] = "body",
  [field_callee] = "callee",
  [field_name] = "name",
  [field_parameters] = "parameters",
  [field_return_type] = "return_type",
  [field_type] = "type",
};

static const TSMapSlice ts_field_map_slices[PRODUCTION_ID_COUNT] = {
  [1] = {.index = 0, .length = 2},
  [2] = {.index = 2, .length = 1},
  [3] = {.index = 3, .length = 4},
};

static const TSFieldMapEntry ts_field_map_entries[] = {
  [0] =
    {field_name, 1},
    {field_type, 3},
  [2] =
    {field_callee, 0},
  [3] =
    {field_body, 5},
    {field_name, 1},
    {field_parameters, 2},
    {field_return_type, 4},
};

static const TSSymbol ts_alias_sequences[PRODUCTION_ID_COUNT][MAX_ALIAS_SEQUENCE_LENGTH] = {
  [0] = {0},
};

static const uint16_t ts_non_terminal_alias_map[] = {
  0,
};

static const TSStateId ts_primary_state_ids[STATE_COUNT] = {
  [0] = 0,
  [1] = 1,
  [2] = 2,
  [3] = 3,
  [4] = 2,
  [5] = 5,
  [6] = 3,
  [7] = 7,
  [8] = 8,
  [9] = 9,
  [10] = 10,
  [11] = 11,
  [12] = 12,
  [13] = 13,
  [14] = 14,
  [15] = 15,
  [16] = 16,
  [17] = 17,
  [18] = 18,
  [19] = 19,
  [20] = 20,
  [21] = 21,
  [22] = 22,
  [23] = 23,
  [24] = 24,
  [25] = 25,
  [26] = 26,
  [27] = 27,
  [28] = 28,
  [29] = 29,
  [30] = 30,
  [31] = 31,
  [32] = 32,
  [33] = 33,
  [34] = 34,
  [35] = 35,
  [36] = 36,
  [37] = 37,
  [38] = 38,
  [39] = 39,
  [40] = 40,
  [41] = 41,
  [42] = 42,
  [43] = 43,
  [44] = 44,
  [45] = 45,
  [46] = 46,
  [47] = 47,
  [48] = 48,
  [49] = 49,
  [50] = 50,
  [51] = 51,
  [52] = 52,
  [53] = 53,
  [54] = 54,
  [55] = 55,
  [56] = 56,
  [57] = 57,
  [58] = 58,
  [59] = 59,
  [60] = 21,
  [61] = 19,
  [62] = 20,
  [63] = 63,
  [64] = 22,
  [65] = 65,
  [66] = 66,
  [67] = 67,
  [68] = 68,
  [69] = 69,
  [70] = 70,
  [71] = 71,
  [72] = 72,
  [73] = 73,
  [74] = 74,
  [75] = 75,
  [76] = 76,
  [77] = 77,
  [78] = 77,
  [79] = 79,
  [80] = 80,
  [81] = 81,
  [82] = 82,
  [83] = 83,
  [84] = 84,
  [85] = 85,
  [86] = 86,
  [87] = 87,
  [88] = 88,
  [89] = 89,
  [90] = 90,
  [91] = 86,
  [92] = 92,
};

static bool ts_lex(TSLexer *lexer, TSStateId state) {
  START_LEXER();
  eof = lexer->eof(lexer);
  switch (state) {
    case 0:
      if (eof) ADVANCE(48);
      if (lookahead == '\r') SKIP(47);
      if (lookahead == '"') ADVANCE(7);
      if (lookahead == '#') ADVANCE(108);
      if (lookahead == '%') ADVANCE(123);
      if (lookahead == '(') ADVANCE(83);
      if (lookahead == ')') ADVANCE(85);
      if (lookahead == '*') ADVANCE(121);
      if (lookahead == '+') ADVANCE(118);
      if (lookahead == ',') ADVANCE(84);
      if (lookahead == '-') ADVANCE(120);
      if (lookahead == '/') ADVANCE(122);
      if (lookahead == ':') ADVANCE(132);
      if (lookahead == ';') ADVANCE(111);
      if (lookahead == '<') ADVANCE(112);
      if (lookahead == '=') ADVANCE(117);
      if (lookahead == '>') ADVANCE(113);
      if (lookahead == 'b') ADVANCE(41);
      if (lookahead == 'e') ADVANCE(35);
      if (lookahead == 'f') ADVANCE(18);
      if (lookahead == 'i') ADVANCE(13);
      if (lookahead == 'l') ADVANCE(28);
      if (lookahead == 'r') ADVANCE(31);
      if (lookahead == 'u') ADVANCE(14);
      if (lookahead == 'v') ADVANCE(26);
      if (lookahead == '{') ADVANCE(106);
      if (lookahead == '}') ADVANCE(107);
      if (lookahead == '\t' ||
          lookahead == '\n' ||
          lookahead == ' ') SKIP(0);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(124);
      END_STATE();
    case 1:
      if (lookahead == '\n') SKIP(4);
      END_STATE();
    case 2:
      if (lookahead == '\n') SKIP(5);
      END_STATE();
    case 3:
      if (lookahead == '\n') SKIP(6);
      END_STATE();
    case 4:
      if (lookahead == '\r') SKIP(1);
      if (lookahead == '"') ADVANCE(7);
      if (lookahead == '#') ADVANCE(108);
      if (lookahead == '%') ADVANCE(123);
      if (lookahead == '(') ADVANCE(83);
      if (lookahead == ')') ADVANCE(85);
      if (lookahead == '*') ADVANCE(121);
      if (lookahead == '+') ADVANCE(118);
      if (lookahead == ',') ADVANCE(84);
      if (lookahead == '-') ADVANCE(119);
      if (lookahead == '/') ADVANCE(122);
      if (lookahead == ';') ADVANCE(111);
      if (lookahead == '<') ADVANCE(112);
      if (lookahead == '=') ADVANCE(117);
      if (lookahead == '>') ADVANCE(113);
      if (lookahead == 'b') ADVANCE(78);
      if (lookahead == 'e') ADVANCE(72);
      if (lookahead == 'i') ADVANCE(68);
      if (lookahead == 'l') ADVANCE(64);
      if (lookahead == 'r') ADVANCE(67);
      if (lookahead == 'v') ADVANCE(63);
      if (lookahead == '{') ADVANCE(106);
      if (lookahead == '}') ADVANCE(107);
      if (lookahead == '\t' ||
          lookahead == '\n' ||
          lookahead == ' ') SKIP(4);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(124);
      if (('A' <= lookahead && lookahead <= 'Z') ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 5:
      if (lookahead == '\r') SKIP(2);
      if (lookahead == '#') ADVANCE(108);
      if (lookahead == '-') ADVANCE(24);
      if (lookahead == 'f') ADVANCE(56);
      if (lookahead == 'i') ADVANCE(51);
      if (lookahead == 'u') ADVANCE(52);
      if (lookahead == '\t' ||
          lookahead == '\n' ||
          lookahead == ' ') SKIP(5);
      if (('A' <= lookahead && lookahead <= 'Z') ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 6:
      if (lookahead == '\r') SKIP(3);
      if (lookahead == '"') ADVANCE(7);
      if (lookahead == '#') ADVANCE(108);
      if (lookahead == '(') ADVANCE(83);
      if (lookahead == ')') ADVANCE(85);
      if (lookahead == ';') ADVANCE(111);
      if (lookahead == '\t' ||
          lookahead == '\n' ||
          lookahead == ' ') SKIP(6);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(124);
      if (('A' <= lookahead && lookahead <= 'Z') ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 7:
      if (lookahead == '"') ADVANCE(126);
      if (lookahead == '\\') ADVANCE(8);
      if (lookahead != 0) ADVANCE(7);
      END_STATE();
    case 8:
      if (lookahead == '"') ADVANCE(127);
      if (lookahead == '\\') ADVANCE(8);
      if (lookahead != 0) ADVANCE(7);
      END_STATE();
    case 9:
      if (lookahead == '#') ADVANCE(10);
      END_STATE();
    case 10:
      if (lookahead == '#') ADVANCE(9);
      if (lookahead == '*') ADVANCE(11);
      if (lookahead != 0) ADVANCE(12);
      END_STATE();
    case 11:
      if (lookahead == '#') ADVANCE(110);
      if (lookahead != 0) ADVANCE(10);
      END_STATE();
    case 12:
      if (lookahead == '*') ADVANCE(11);
      if (lookahead != 0) ADVANCE(12);
      END_STATE();
    case 13:
      if (lookahead == '1') ADVANCE(22);
      if (lookahead == '3') ADVANCE(16);
      if (lookahead == '6') ADVANCE(20);
      if (lookahead == '8') ADVANCE(88);
      if (lookahead == 'f') ADVANCE(139);
      END_STATE();
    case 14:
      if (lookahead == '1') ADVANCE(23);
      if (lookahead == '3') ADVANCE(17);
      if (lookahead == '6') ADVANCE(21);
      if (lookahead == '8') ADVANCE(86);
      END_STATE();
    case 15:
      if (lookahead == '2') ADVANCE(102);
      END_STATE();
    case 16:
      if (lookahead == '2') ADVANCE(96);
      END_STATE();
    case 17:
      if (lookahead == '2') ADVANCE(94);
      END_STATE();
    case 18:
      if (lookahead == '3') ADVANCE(15);
      if (lookahead == '6') ADVANCE(19);
      if (lookahead == 'u') ADVANCE(36);
      END_STATE();
    case 19:
      if (lookahead == '4') ADVANCE(104);
      END_STATE();
    case 20:
      if (lookahead == '4') ADVANCE(100);
      END_STATE();
    case 21:
      if (lookahead == '4') ADVANCE(98);
      END_STATE();
    case 22:
      if (lookahead == '6') ADVANCE(92);
      END_STATE();
    case 23:
      if (lookahead == '6') ADVANCE(90);
      END_STATE();
    case 24:
      if (lookahead == '>') ADVANCE(50);
      END_STATE();
    case 25:
      if (lookahead == 'a') ADVANCE(34);
      END_STATE();
    case 26:
      if (lookahead == 'a') ADVANCE(40);
      END_STATE();
    case 27:
      if (lookahead == 'c') ADVANCE(49);
      END_STATE();
    case 28:
      if (lookahead == 'e') ADVANCE(43);
      if (lookahead == 'o') ADVANCE(38);
      END_STATE();
    case 29:
      if (lookahead == 'e') ADVANCE(143);
      END_STATE();
    case 30:
      if (lookahead == 'e') ADVANCE(25);
      END_STATE();
    case 31:
      if (lookahead == 'e') ADVANCE(44);
      END_STATE();
    case 32:
      if (lookahead == 'f') ADVANCE(141);
      END_STATE();
    case 33:
      if (lookahead == 'i') ADVANCE(32);
      if (lookahead == 's') ADVANCE(29);
      END_STATE();
    case 34:
      if (lookahead == 'k') ADVANCE(135);
      END_STATE();
    case 35:
      if (lookahead == 'l') ADVANCE(33);
      END_STATE();
    case 36:
      if (lookahead == 'n') ADVANCE(27);
      END_STATE();
    case 37:
      if (lookahead == 'n') ADVANCE(133);
      END_STATE();
    case 38:
      if (lookahead == 'o') ADVANCE(39);
      END_STATE();
    case 39:
      if (lookahead == 'p') ADVANCE(137);
      END_STATE();
    case 40:
      if (lookahead == 'r') ADVANCE(128);
      END_STATE();
    case 41:
      if (lookahead == 'r') ADVANCE(30);
      END_STATE();
    case 42:
      if (lookahead == 'r') ADVANCE(37);
      END_STATE();
    case 43:
      if (lookahead == 't') ADVANCE(130);
      END_STATE();
    case 44:
      if (lookahead == 't') ADVANCE(45);
      END_STATE();
    case 45:
      if (lookahead == 'u') ADVANCE(42);
      END_STATE();
    case 46:
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(125);
      END_STATE();
    case 47:
      if (eof) ADVANCE(48);
      if (lookahead == '\n') SKIP(0);
      END_STATE();
    case 48:
      ACCEPT_TOKEN(ts_builtin_sym_end);
      END_STATE();
    case 49:
      ACCEPT_TOKEN(anon_sym_func);
      END_STATE();
    case 50:
      ACCEPT_TOKEN(anon_sym_DASH_GT);
      END_STATE();
    case 51:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == '1') ADVANCE(60);
      if (lookahead == '3') ADVANCE(54);
      if (lookahead == '6') ADVANCE(58);
      if (lookahead == '8') ADVANCE(89);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 52:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == '1') ADVANCE(61);
      if (lookahead == '3') ADVANCE(55);
      if (lookahead == '6') ADVANCE(59);
      if (lookahead == '8') ADVANCE(87);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 53:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == '2') ADVANCE(103);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 54:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == '2') ADVANCE(97);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 55:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == '2') ADVANCE(95);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 56:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == '3') ADVANCE(53);
      if (lookahead == '6') ADVANCE(57);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 57:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == '4') ADVANCE(105);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 58:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == '4') ADVANCE(101);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 59:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == '4') ADVANCE(99);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 60:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == '6') ADVANCE(93);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 61:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == '6') ADVANCE(91);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 62:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'a') ADVANCE(71);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 63:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'a') ADVANCE(76);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('b' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 64:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'e') ADVANCE(79);
      if (lookahead == 'o') ADVANCE(74);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 65:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'e') ADVANCE(144);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 66:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'e') ADVANCE(62);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 67:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'e') ADVANCE(80);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 68:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'f') ADVANCE(140);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 69:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'f') ADVANCE(142);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 70:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'i') ADVANCE(69);
      if (lookahead == 's') ADVANCE(65);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 71:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'k') ADVANCE(136);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 72:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'l') ADVANCE(70);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 73:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'n') ADVANCE(134);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 74:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'o') ADVANCE(75);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 75:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'p') ADVANCE(138);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 76:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'r') ADVANCE(129);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 77:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'r') ADVANCE(73);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 78:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'r') ADVANCE(66);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 79:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 't') ADVANCE(131);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 80:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 't') ADVANCE(81);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 81:
      ACCEPT_TOKEN(sym_identifier);
      if (lookahead == 'u') ADVANCE(77);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 82:
      ACCEPT_TOKEN(sym_identifier);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 83:
      ACCEPT_TOKEN(anon_sym_LPAREN);
      END_STATE();
    case 84:
      ACCEPT_TOKEN(anon_sym_COMMA);
      END_STATE();
    case 85:
      ACCEPT_TOKEN(anon_sym_RPAREN);
      END_STATE();
    case 86:
      ACCEPT_TOKEN(anon_sym_u8);
      END_STATE();
    case 87:
      ACCEPT_TOKEN(anon_sym_u8);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 88:
      ACCEPT_TOKEN(anon_sym_i8);
      END_STATE();
    case 89:
      ACCEPT_TOKEN(anon_sym_i8);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 90:
      ACCEPT_TOKEN(anon_sym_u16);
      END_STATE();
    case 91:
      ACCEPT_TOKEN(anon_sym_u16);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 92:
      ACCEPT_TOKEN(anon_sym_i16);
      END_STATE();
    case 93:
      ACCEPT_TOKEN(anon_sym_i16);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 94:
      ACCEPT_TOKEN(anon_sym_u32);
      END_STATE();
    case 95:
      ACCEPT_TOKEN(anon_sym_u32);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 96:
      ACCEPT_TOKEN(anon_sym_i32);
      END_STATE();
    case 97:
      ACCEPT_TOKEN(anon_sym_i32);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 98:
      ACCEPT_TOKEN(anon_sym_u64);
      END_STATE();
    case 99:
      ACCEPT_TOKEN(anon_sym_u64);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 100:
      ACCEPT_TOKEN(anon_sym_i64);
      END_STATE();
    case 101:
      ACCEPT_TOKEN(anon_sym_i64);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 102:
      ACCEPT_TOKEN(anon_sym_f32);
      END_STATE();
    case 103:
      ACCEPT_TOKEN(anon_sym_f32);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 104:
      ACCEPT_TOKEN(anon_sym_f64);
      END_STATE();
    case 105:
      ACCEPT_TOKEN(anon_sym_f64);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 106:
      ACCEPT_TOKEN(anon_sym_LBRACE);
      END_STATE();
    case 107:
      ACCEPT_TOKEN(anon_sym_RBRACE);
      END_STATE();
    case 108:
      ACCEPT_TOKEN(sym_oneline_comment);
      if (lookahead == '*') ADVANCE(10);
      if (lookahead != 0) ADVANCE(109);
      END_STATE();
    case 109:
      ACCEPT_TOKEN(sym_oneline_comment);
      if (lookahead != 0 &&
          lookahead != '\n') ADVANCE(109);
      END_STATE();
    case 110:
      ACCEPT_TOKEN(sym_multiline_comment);
      END_STATE();
    case 111:
      ACCEPT_TOKEN(anon_sym_SEMI);
      END_STATE();
    case 112:
      ACCEPT_TOKEN(anon_sym_LT);
      if (lookahead == '=') ADVANCE(114);
      END_STATE();
    case 113:
      ACCEPT_TOKEN(anon_sym_GT);
      if (lookahead == '=') ADVANCE(115);
      END_STATE();
    case 114:
      ACCEPT_TOKEN(anon_sym_LT_EQ);
      END_STATE();
    case 115:
      ACCEPT_TOKEN(anon_sym_GT_EQ);
      END_STATE();
    case 116:
      ACCEPT_TOKEN(anon_sym_EQ_EQ);
      END_STATE();
    case 117:
      ACCEPT_TOKEN(anon_sym_EQ);
      if (lookahead == '=') ADVANCE(116);
      END_STATE();
    case 118:
      ACCEPT_TOKEN(anon_sym_PLUS);
      END_STATE();
    case 119:
      ACCEPT_TOKEN(anon_sym_DASH);
      END_STATE();
    case 120:
      ACCEPT_TOKEN(anon_sym_DASH);
      if (lookahead == '>') ADVANCE(50);
      END_STATE();
    case 121:
      ACCEPT_TOKEN(anon_sym_STAR);
      END_STATE();
    case 122:
      ACCEPT_TOKEN(anon_sym_SLASH);
      END_STATE();
    case 123:
      ACCEPT_TOKEN(anon_sym_PERCENT);
      END_STATE();
    case 124:
      ACCEPT_TOKEN(sym_integer_literal);
      if (lookahead == '.') ADVANCE(46);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(124);
      END_STATE();
    case 125:
      ACCEPT_TOKEN(sym_floating_point_literal);
      if (('0' <= lookahead && lookahead <= '9')) ADVANCE(125);
      END_STATE();
    case 126:
      ACCEPT_TOKEN(sym_string_literal);
      END_STATE();
    case 127:
      ACCEPT_TOKEN(sym_string_literal);
      if (lookahead == '"') ADVANCE(126);
      if (lookahead == '\\') ADVANCE(8);
      if (lookahead != 0) ADVANCE(7);
      END_STATE();
    case 128:
      ACCEPT_TOKEN(anon_sym_var);
      END_STATE();
    case 129:
      ACCEPT_TOKEN(anon_sym_var);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 130:
      ACCEPT_TOKEN(anon_sym_let);
      END_STATE();
    case 131:
      ACCEPT_TOKEN(anon_sym_let);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 132:
      ACCEPT_TOKEN(anon_sym_COLON);
      END_STATE();
    case 133:
      ACCEPT_TOKEN(anon_sym_return);
      END_STATE();
    case 134:
      ACCEPT_TOKEN(anon_sym_return);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 135:
      ACCEPT_TOKEN(anon_sym_break);
      END_STATE();
    case 136:
      ACCEPT_TOKEN(anon_sym_break);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 137:
      ACCEPT_TOKEN(anon_sym_loop);
      END_STATE();
    case 138:
      ACCEPT_TOKEN(anon_sym_loop);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 139:
      ACCEPT_TOKEN(anon_sym_if);
      END_STATE();
    case 140:
      ACCEPT_TOKEN(anon_sym_if);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 141:
      ACCEPT_TOKEN(anon_sym_elif);
      END_STATE();
    case 142:
      ACCEPT_TOKEN(anon_sym_elif);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    case 143:
      ACCEPT_TOKEN(anon_sym_else);
      END_STATE();
    case 144:
      ACCEPT_TOKEN(anon_sym_else);
      if (('0' <= lookahead && lookahead <= '9') ||
          ('A' <= lookahead && lookahead <= 'Z') ||
          lookahead == '_' ||
          ('a' <= lookahead && lookahead <= 'z')) ADVANCE(82);
      END_STATE();
    default:
      return false;
  }
}

static const TSLexerMode ts_lex_modes[STATE_COUNT] = {
  [0] = {.lex_state = 0},
  [1] = {.lex_state = 0},
  [2] = {.lex_state = 4},
  [3] = {.lex_state = 4},
  [4] = {.lex_state = 4},
  [5] = {.lex_state = 4},
  [6] = {.lex_state = 4},
  [7] = {.lex_state = 4},
  [8] = {.lex_state = 4},
  [9] = {.lex_state = 4},
  [10] = {.lex_state = 4},
  [11] = {.lex_state = 4},
  [12] = {.lex_state = 4},
  [13] = {.lex_state = 4},
  [14] = {.lex_state = 4},
  [15] = {.lex_state = 4},
  [16] = {.lex_state = 4},
  [17] = {.lex_state = 4},
  [18] = {.lex_state = 4},
  [19] = {.lex_state = 4},
  [20] = {.lex_state = 4},
  [21] = {.lex_state = 4},
  [22] = {.lex_state = 4},
  [23] = {.lex_state = 4},
  [24] = {.lex_state = 4},
  [25] = {.lex_state = 4},
  [26] = {.lex_state = 4},
  [27] = {.lex_state = 4},
  [28] = {.lex_state = 4},
  [29] = {.lex_state = 4},
  [30] = {.lex_state = 4},
  [31] = {.lex_state = 5},
  [32] = {.lex_state = 5},
  [33] = {.lex_state = 6},
  [34] = {.lex_state = 4},
  [35] = {.lex_state = 6},
  [36] = {.lex_state = 4},
  [37] = {.lex_state = 4},
  [38] = {.lex_state = 6},
  [39] = {.lex_state = 6},
  [40] = {.lex_state = 6},
  [41] = {.lex_state = 0},
  [42] = {.lex_state = 6},
  [43] = {.lex_state = 6},
  [44] = {.lex_state = 4},
  [45] = {.lex_state = 6},
  [46] = {.lex_state = 4},
  [47] = {.lex_state = 4},
  [48] = {.lex_state = 0},
  [49] = {.lex_state = 6},
  [50] = {.lex_state = 6},
  [51] = {.lex_state = 4},
  [52] = {.lex_state = 6},
  [53] = {.lex_state = 4},
  [54] = {.lex_state = 4},
  [55] = {.lex_state = 0},
  [56] = {.lex_state = 0},
  [57] = {.lex_state = 0},
  [58] = {.lex_state = 0},
  [59] = {.lex_state = 0},
  [60] = {.lex_state = 0},
  [61] = {.lex_state = 0},
  [62] = {.lex_state = 0},
  [63] = {.lex_state = 0},
  [64] = {.lex_state = 0},
  [65] = {.lex_state = 0},
  [66] = {.lex_state = 0},
  [67] = {.lex_state = 0},
  [68] = {.lex_state = 0},
  [69] = {.lex_state = 0},
  [70] = {.lex_state = 0},
  [71] = {.lex_state = 0},
  [72] = {.lex_state = 0},
  [73] = {.lex_state = 0},
  [74] = {.lex_state = 0},
  [75] = {.lex_state = 0},
  [76] = {.lex_state = 0},
  [77] = {.lex_state = 0},
  [78] = {.lex_state = 0},
  [79] = {.lex_state = 0},
  [80] = {.lex_state = 5},
  [81] = {.lex_state = 0},
  [82] = {.lex_state = 0},
  [83] = {.lex_state = 6},
  [84] = {.lex_state = 0},
  [85] = {.lex_state = 6},
  [86] = {.lex_state = 0},
  [87] = {.lex_state = 0},
  [88] = {.lex_state = 5},
  [89] = {.lex_state = 0},
  [90] = {.lex_state = 5},
  [91] = {.lex_state = 0},
  [92] = {.lex_state = 5},
};

static const uint16_t ts_parse_table[LARGE_STATE_COUNT][SYMBOL_COUNT] = {
  [STATE(0)] = {
    [ts_builtin_sym_end] = ACTIONS(1),
    [anon_sym_func] = ACTIONS(1),
    [anon_sym_DASH_GT] = ACTIONS(1),
    [anon_sym_LPAREN] = ACTIONS(1),
    [anon_sym_COMMA] = ACTIONS(1),
    [anon_sym_RPAREN] = ACTIONS(1),
    [anon_sym_u8] = ACTIONS(1),
    [anon_sym_i8] = ACTIONS(1),
    [anon_sym_u16] = ACTIONS(1),
    [anon_sym_i16] = ACTIONS(1),
    [anon_sym_u32] = ACTIONS(1),
    [anon_sym_i32] = ACTIONS(1),
    [anon_sym_u64] = ACTIONS(1),
    [anon_sym_i64] = ACTIONS(1),
    [anon_sym_f32] = ACTIONS(1),
    [anon_sym_f64] = ACTIONS(1),
    [anon_sym_LBRACE] = ACTIONS(1),
    [anon_sym_RBRACE] = ACTIONS(1),
    [sym_oneline_comment] = ACTIONS(3),
    [sym_multiline_comment] = ACTIONS(5),
    [anon_sym_SEMI] = ACTIONS(1),
    [anon_sym_LT] = ACTIONS(1),
    [anon_sym_GT] = ACTIONS(1),
    [anon_sym_LT_EQ] = ACTIONS(1),
    [anon_sym_GT_EQ] = ACTIONS(1),
    [anon_sym_EQ_EQ] = ACTIONS(1),
    [anon_sym_EQ] = ACTIONS(1),
    [anon_sym_PLUS] = ACTIONS(1),
    [anon_sym_DASH] = ACTIONS(1),
    [anon_sym_STAR] = ACTIONS(1),
    [anon_sym_SLASH] = ACTIONS(1),
    [anon_sym_PERCENT] = ACTIONS(1),
    [sym_integer_literal] = ACTIONS(1),
    [sym_floating_point_literal] = ACTIONS(1),
    [sym_string_literal] = ACTIONS(1),
    [anon_sym_var] = ACTIONS(1),
    [anon_sym_let] = ACTIONS(1),
    [anon_sym_COLON] = ACTIONS(1),
    [anon_sym_return] = ACTIONS(1),
    [anon_sym_break] = ACTIONS(1),
    [anon_sym_loop] = ACTIONS(1),
    [anon_sym_if] = ACTIONS(1),
    [anon_sym_elif] = ACTIONS(1),
    [anon_sym_else] = ACTIONS(1),
  },
  [STATE(1)] = {
    [sym_source_file] = STATE(79),
    [sym__definition] = STATE(41),
    [sym__declaration] = STATE(41),
    [sym_function_definition] = STATE(41),
    [sym_variable_declaration_sentence] = STATE(41),
    [sym_variable_definition_sentence] = STATE(41),
    [sym_variable_declaration] = STATE(78),
    [sym_variable_definition] = STATE(86),
    [aux_sym_source_file_repeat1] = STATE(41),
    [ts_builtin_sym_end] = ACTIONS(7),
    [anon_sym_func] = ACTIONS(9),
    [sym_oneline_comment] = ACTIONS(3),
    [sym_multiline_comment] = ACTIONS(5),
    [anon_sym_var] = ACTIONS(11),
    [anon_sym_let] = ACTIONS(11),
  },
};

static const uint16_t ts_small_parse_table[] = {
  [0] = 20,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(13), 1,
      sym_identifier,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(17), 1,
      anon_sym_LBRACE,
    ACTIONS(19), 1,
      anon_sym_RBRACE,
    ACTIONS(21), 1,
      sym_integer_literal,
    ACTIONS(27), 1,
      anon_sym_return,
    ACTIONS(29), 1,
      anon_sym_break,
    ACTIONS(31), 1,
      anon_sym_loop,
    ACTIONS(33), 1,
      anon_sym_if,
    ACTIONS(35), 1,
      anon_sym_elif,
    ACTIONS(37), 1,
      anon_sym_else,
    STATE(77), 1,
      sym_variable_declaration,
    STATE(91), 1,
      sym_variable_definition,
    ACTIONS(23), 2,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(25), 2,
      anon_sym_var,
      anon_sym_let,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(44), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
    STATE(5), 13,
      sym_block,
      sym__sentence,
      sym_expression_sentence,
      sym_variable_declaration_sentence,
      sym_variable_definition_sentence,
      sym_return_sentence,
      sym_break_sentence,
      sym_loop_statement,
      sym_if_statement,
      sym_elif_statement,
      sym_else_statement,
      sym__blocky,
      aux_sym_block_repeat1,
  [80] = 20,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(13), 1,
      sym_identifier,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(17), 1,
      anon_sym_LBRACE,
    ACTIONS(21), 1,
      sym_integer_literal,
    ACTIONS(27), 1,
      anon_sym_return,
    ACTIONS(29), 1,
      anon_sym_break,
    ACTIONS(31), 1,
      anon_sym_loop,
    ACTIONS(33), 1,
      anon_sym_if,
    ACTIONS(35), 1,
      anon_sym_elif,
    ACTIONS(37), 1,
      anon_sym_else,
    ACTIONS(39), 1,
      anon_sym_RBRACE,
    STATE(77), 1,
      sym_variable_declaration,
    STATE(91), 1,
      sym_variable_definition,
    ACTIONS(23), 2,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(25), 2,
      anon_sym_var,
      anon_sym_let,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(44), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
    STATE(2), 13,
      sym_block,
      sym__sentence,
      sym_expression_sentence,
      sym_variable_declaration_sentence,
      sym_variable_definition_sentence,
      sym_return_sentence,
      sym_break_sentence,
      sym_loop_statement,
      sym_if_statement,
      sym_elif_statement,
      sym_else_statement,
      sym__blocky,
      aux_sym_block_repeat1,
  [160] = 20,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(13), 1,
      sym_identifier,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(17), 1,
      anon_sym_LBRACE,
    ACTIONS(21), 1,
      sym_integer_literal,
    ACTIONS(27), 1,
      anon_sym_return,
    ACTIONS(29), 1,
      anon_sym_break,
    ACTIONS(31), 1,
      anon_sym_loop,
    ACTIONS(33), 1,
      anon_sym_if,
    ACTIONS(35), 1,
      anon_sym_elif,
    ACTIONS(37), 1,
      anon_sym_else,
    ACTIONS(41), 1,
      anon_sym_RBRACE,
    STATE(77), 1,
      sym_variable_declaration,
    STATE(91), 1,
      sym_variable_definition,
    ACTIONS(23), 2,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(25), 2,
      anon_sym_var,
      anon_sym_let,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(44), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
    STATE(5), 13,
      sym_block,
      sym__sentence,
      sym_expression_sentence,
      sym_variable_declaration_sentence,
      sym_variable_definition_sentence,
      sym_return_sentence,
      sym_break_sentence,
      sym_loop_statement,
      sym_if_statement,
      sym_elif_statement,
      sym_else_statement,
      sym__blocky,
      aux_sym_block_repeat1,
  [240] = 20,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(43), 1,
      sym_identifier,
    ACTIONS(46), 1,
      anon_sym_LPAREN,
    ACTIONS(49), 1,
      anon_sym_LBRACE,
    ACTIONS(52), 1,
      anon_sym_RBRACE,
    ACTIONS(54), 1,
      sym_integer_literal,
    ACTIONS(63), 1,
      anon_sym_return,
    ACTIONS(66), 1,
      anon_sym_break,
    ACTIONS(69), 1,
      anon_sym_loop,
    ACTIONS(72), 1,
      anon_sym_if,
    ACTIONS(75), 1,
      anon_sym_elif,
    ACTIONS(78), 1,
      anon_sym_else,
    STATE(77), 1,
      sym_variable_declaration,
    STATE(91), 1,
      sym_variable_definition,
    ACTIONS(57), 2,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(60), 2,
      anon_sym_var,
      anon_sym_let,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(44), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
    STATE(5), 13,
      sym_block,
      sym__sentence,
      sym_expression_sentence,
      sym_variable_declaration_sentence,
      sym_variable_definition_sentence,
      sym_return_sentence,
      sym_break_sentence,
      sym_loop_statement,
      sym_if_statement,
      sym_elif_statement,
      sym_else_statement,
      sym__blocky,
      aux_sym_block_repeat1,
  [320] = 20,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(13), 1,
      sym_identifier,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(17), 1,
      anon_sym_LBRACE,
    ACTIONS(21), 1,
      sym_integer_literal,
    ACTIONS(27), 1,
      anon_sym_return,
    ACTIONS(29), 1,
      anon_sym_break,
    ACTIONS(31), 1,
      anon_sym_loop,
    ACTIONS(33), 1,
      anon_sym_if,
    ACTIONS(35), 1,
      anon_sym_elif,
    ACTIONS(37), 1,
      anon_sym_else,
    ACTIONS(81), 1,
      anon_sym_RBRACE,
    STATE(77), 1,
      sym_variable_declaration,
    STATE(91), 1,
      sym_variable_definition,
    ACTIONS(23), 2,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(25), 2,
      anon_sym_var,
      anon_sym_let,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(44), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
    STATE(4), 13,
      sym_block,
      sym__sentence,
      sym_expression_sentence,
      sym_variable_declaration_sentence,
      sym_variable_definition_sentence,
      sym_return_sentence,
      sym_break_sentence,
      sym_loop_statement,
      sym_if_statement,
      sym_elif_statement,
      sym_else_statement,
      sym__blocky,
      aux_sym_block_repeat1,
  [400] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(85), 3,
      anon_sym_LT,
      anon_sym_GT,
      anon_sym_EQ,
    ACTIONS(83), 12,
      anon_sym_LPAREN,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_SEMI,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
      anon_sym_EQ_EQ,
      anon_sym_PLUS,
      anon_sym_DASH,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [426] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(89), 3,
      anon_sym_LT,
      anon_sym_GT,
      anon_sym_EQ,
    ACTIONS(87), 12,
      anon_sym_LPAREN,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_SEMI,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
      anon_sym_EQ_EQ,
      anon_sym_PLUS,
      anon_sym_DASH,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [452] = 5,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(91), 1,
      anon_sym_LPAREN,
    ACTIONS(95), 3,
      anon_sym_LT,
      anon_sym_GT,
      anon_sym_EQ,
    ACTIONS(93), 11,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_SEMI,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
      anon_sym_EQ_EQ,
      anon_sym_PLUS,
      anon_sym_DASH,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [480] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(99), 3,
      anon_sym_LT,
      anon_sym_GT,
      anon_sym_EQ,
    ACTIONS(97), 12,
      anon_sym_LPAREN,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_SEMI,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
      anon_sym_EQ_EQ,
      anon_sym_PLUS,
      anon_sym_DASH,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [506] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(103), 3,
      anon_sym_LT,
      anon_sym_GT,
      anon_sym_EQ,
    ACTIONS(101), 12,
      anon_sym_LPAREN,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_SEMI,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
      anon_sym_EQ_EQ,
      anon_sym_PLUS,
      anon_sym_DASH,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [532] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(107), 3,
      anon_sym_LT,
      anon_sym_GT,
      anon_sym_EQ,
    ACTIONS(105), 12,
      anon_sym_LPAREN,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_SEMI,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
      anon_sym_EQ_EQ,
      anon_sym_PLUS,
      anon_sym_DASH,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [558] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(111), 5,
      anon_sym_LPAREN,
      anon_sym_LBRACE,
      anon_sym_RBRACE,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(109), 10,
      sym_identifier,
      sym_integer_literal,
      anon_sym_var,
      anon_sym_let,
      anon_sym_return,
      anon_sym_break,
      anon_sym_loop,
      anon_sym_if,
      anon_sym_elif,
      anon_sym_else,
  [584] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(115), 5,
      anon_sym_LPAREN,
      anon_sym_LBRACE,
      anon_sym_RBRACE,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(113), 10,
      sym_identifier,
      sym_integer_literal,
      anon_sym_var,
      anon_sym_let,
      anon_sym_return,
      anon_sym_break,
      anon_sym_loop,
      anon_sym_if,
      anon_sym_elif,
      anon_sym_else,
  [610] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(119), 5,
      anon_sym_LPAREN,
      anon_sym_LBRACE,
      anon_sym_RBRACE,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(117), 10,
      sym_identifier,
      sym_integer_literal,
      anon_sym_var,
      anon_sym_let,
      anon_sym_return,
      anon_sym_break,
      anon_sym_loop,
      anon_sym_if,
      anon_sym_elif,
      anon_sym_else,
  [636] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(123), 5,
      anon_sym_LPAREN,
      anon_sym_LBRACE,
      anon_sym_RBRACE,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(121), 10,
      sym_identifier,
      sym_integer_literal,
      anon_sym_var,
      anon_sym_let,
      anon_sym_return,
      anon_sym_break,
      anon_sym_loop,
      anon_sym_if,
      anon_sym_elif,
      anon_sym_else,
  [662] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(127), 5,
      anon_sym_LPAREN,
      anon_sym_LBRACE,
      anon_sym_RBRACE,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(125), 10,
      sym_identifier,
      sym_integer_literal,
      anon_sym_var,
      anon_sym_let,
      anon_sym_return,
      anon_sym_break,
      anon_sym_loop,
      anon_sym_if,
      anon_sym_elif,
      anon_sym_else,
  [688] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(131), 5,
      anon_sym_LPAREN,
      anon_sym_LBRACE,
      anon_sym_RBRACE,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(129), 10,
      sym_identifier,
      sym_integer_literal,
      anon_sym_var,
      anon_sym_let,
      anon_sym_return,
      anon_sym_break,
      anon_sym_loop,
      anon_sym_if,
      anon_sym_elif,
      anon_sym_else,
  [714] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(135), 5,
      anon_sym_LPAREN,
      anon_sym_LBRACE,
      anon_sym_RBRACE,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(133), 10,
      sym_identifier,
      sym_integer_literal,
      anon_sym_var,
      anon_sym_let,
      anon_sym_return,
      anon_sym_break,
      anon_sym_loop,
      anon_sym_if,
      anon_sym_elif,
      anon_sym_else,
  [740] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(139), 5,
      anon_sym_LPAREN,
      anon_sym_LBRACE,
      anon_sym_RBRACE,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(137), 10,
      sym_identifier,
      sym_integer_literal,
      anon_sym_var,
      anon_sym_let,
      anon_sym_return,
      anon_sym_break,
      anon_sym_loop,
      anon_sym_if,
      anon_sym_elif,
      anon_sym_else,
  [766] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(143), 5,
      anon_sym_LPAREN,
      anon_sym_LBRACE,
      anon_sym_RBRACE,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(141), 10,
      sym_identifier,
      sym_integer_literal,
      anon_sym_var,
      anon_sym_let,
      anon_sym_return,
      anon_sym_break,
      anon_sym_loop,
      anon_sym_if,
      anon_sym_elif,
      anon_sym_else,
  [792] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(147), 5,
      anon_sym_LPAREN,
      anon_sym_LBRACE,
      anon_sym_RBRACE,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(145), 10,
      sym_identifier,
      sym_integer_literal,
      anon_sym_var,
      anon_sym_let,
      anon_sym_return,
      anon_sym_break,
      anon_sym_loop,
      anon_sym_if,
      anon_sym_elif,
      anon_sym_else,
  [818] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(151), 5,
      anon_sym_LPAREN,
      anon_sym_LBRACE,
      anon_sym_RBRACE,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(149), 10,
      sym_identifier,
      sym_integer_literal,
      anon_sym_var,
      anon_sym_let,
      anon_sym_return,
      anon_sym_break,
      anon_sym_loop,
      anon_sym_if,
      anon_sym_elif,
      anon_sym_else,
  [844] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(155), 5,
      anon_sym_LPAREN,
      anon_sym_LBRACE,
      anon_sym_RBRACE,
      sym_floating_point_literal,
      sym_string_literal,
    ACTIONS(153), 10,
      sym_identifier,
      sym_integer_literal,
      anon_sym_var,
      anon_sym_let,
      anon_sym_return,
      anon_sym_break,
      anon_sym_loop,
      anon_sym_if,
      anon_sym_elif,
      anon_sym_else,
  [870] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(159), 3,
      anon_sym_LT,
      anon_sym_GT,
      anon_sym_EQ,
    ACTIONS(157), 11,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_SEMI,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
      anon_sym_EQ_EQ,
      anon_sym_PLUS,
      anon_sym_DASH,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [895] = 11,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(161), 1,
      anon_sym_COMMA,
    ACTIONS(163), 1,
      anon_sym_RPAREN,
    ACTIONS(169), 1,
      anon_sym_EQ_EQ,
    ACTIONS(171), 1,
      anon_sym_EQ,
    STATE(66), 1,
      aux_sym_function_call_repeat1,
    ACTIONS(165), 2,
      anon_sym_LT,
      anon_sym_GT,
    ACTIONS(167), 2,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
    ACTIONS(173), 2,
      anon_sym_PLUS,
      anon_sym_DASH,
    ACTIONS(175), 3,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [934] = 8,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(169), 1,
      anon_sym_EQ_EQ,
    ACTIONS(171), 1,
      anon_sym_EQ,
    ACTIONS(159), 2,
      anon_sym_LT,
      anon_sym_GT,
    ACTIONS(173), 2,
      anon_sym_PLUS,
      anon_sym_DASH,
    ACTIONS(175), 3,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
    ACTIONS(157), 5,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_SEMI,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
  [967] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(179), 3,
      anon_sym_LT,
      anon_sym_GT,
      anon_sym_EQ,
    ACTIONS(177), 11,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_SEMI,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
      anon_sym_EQ_EQ,
      anon_sym_PLUS,
      anon_sym_DASH,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [992] = 5,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(159), 3,
      anon_sym_LT,
      anon_sym_GT,
      anon_sym_EQ,
    ACTIONS(175), 3,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
    ACTIONS(157), 8,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_SEMI,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
      anon_sym_EQ_EQ,
      anon_sym_PLUS,
      anon_sym_DASH,
  [1019] = 6,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(173), 2,
      anon_sym_PLUS,
      anon_sym_DASH,
    ACTIONS(159), 3,
      anon_sym_LT,
      anon_sym_GT,
      anon_sym_EQ,
    ACTIONS(175), 3,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
    ACTIONS(157), 6,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_SEMI,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
      anon_sym_EQ_EQ,
  [1048] = 5,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(181), 1,
      sym_identifier,
    STATE(63), 3,
      sym__type,
      sym_builtin_type,
      sym_user_type,
    ACTIONS(183), 10,
      anon_sym_u8,
      anon_sym_i8,
      anon_sym_u16,
      anon_sym_i16,
      anon_sym_u32,
      anon_sym_i32,
      anon_sym_u64,
      anon_sym_i64,
      anon_sym_f32,
      anon_sym_f64,
  [1075] = 5,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(181), 1,
      sym_identifier,
    STATE(72), 3,
      sym__type,
      sym_builtin_type,
      sym_user_type,
    ACTIONS(183), 10,
      anon_sym_u8,
      anon_sym_i8,
      anon_sym_u16,
      anon_sym_i16,
      anon_sym_u32,
      anon_sym_i32,
      anon_sym_u64,
      anon_sym_i64,
      anon_sym_f32,
      anon_sym_f64,
  [1102] = 9,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(185), 1,
      sym_identifier,
    ACTIONS(187), 1,
      anon_sym_RPAREN,
    ACTIONS(189), 1,
      sym_integer_literal,
    ACTIONS(191), 2,
      sym_floating_point_literal,
      sym_string_literal,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(26), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
  [1136] = 6,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(91), 1,
      anon_sym_LPAREN,
    ACTIONS(193), 1,
      anon_sym_RPAREN,
    ACTIONS(95), 3,
      anon_sym_LT,
      anon_sym_GT,
      anon_sym_EQ,
    ACTIONS(93), 8,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
      anon_sym_EQ_EQ,
      anon_sym_PLUS,
      anon_sym_DASH,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [1164] = 9,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(185), 1,
      sym_identifier,
    ACTIONS(195), 1,
      anon_sym_SEMI,
    ACTIONS(197), 1,
      sym_integer_literal,
    ACTIONS(199), 2,
      sym_floating_point_literal,
      sym_string_literal,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(46), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
  [1198] = 9,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(169), 1,
      anon_sym_EQ_EQ,
    ACTIONS(171), 1,
      anon_sym_EQ,
    ACTIONS(165), 2,
      anon_sym_LT,
      anon_sym_GT,
    ACTIONS(167), 2,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
    ACTIONS(173), 2,
      anon_sym_PLUS,
      anon_sym_DASH,
    ACTIONS(201), 2,
      anon_sym_COMMA,
      anon_sym_RPAREN,
    ACTIONS(175), 3,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [1232] = 9,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(169), 1,
      anon_sym_EQ_EQ,
    ACTIONS(171), 1,
      anon_sym_EQ,
    ACTIONS(203), 1,
      anon_sym_RPAREN,
    ACTIONS(165), 2,
      anon_sym_LT,
      anon_sym_GT,
    ACTIONS(167), 2,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
    ACTIONS(173), 2,
      anon_sym_PLUS,
      anon_sym_DASH,
    ACTIONS(175), 3,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [1265] = 8,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(185), 1,
      sym_identifier,
    ACTIONS(205), 1,
      sym_integer_literal,
    ACTIONS(207), 2,
      sym_floating_point_literal,
      sym_string_literal,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(27), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
  [1296] = 8,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(185), 1,
      sym_identifier,
    ACTIONS(209), 1,
      sym_integer_literal,
    ACTIONS(211), 2,
      sym_floating_point_literal,
      sym_string_literal,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(30), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
  [1327] = 8,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(185), 1,
      sym_identifier,
    ACTIONS(213), 1,
      sym_integer_literal,
    ACTIONS(215), 2,
      sym_floating_point_literal,
      sym_string_literal,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(29), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
  [1358] = 8,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(9), 1,
      anon_sym_func,
    ACTIONS(217), 1,
      ts_builtin_sym_end,
    STATE(78), 1,
      sym_variable_declaration,
    STATE(86), 1,
      sym_variable_definition,
    ACTIONS(11), 2,
      anon_sym_var,
      anon_sym_let,
    STATE(48), 6,
      sym__definition,
      sym__declaration,
      sym_function_definition,
      sym_variable_declaration_sentence,
      sym_variable_definition_sentence,
      aux_sym_source_file_repeat1,
  [1389] = 8,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(185), 1,
      sym_identifier,
    ACTIONS(219), 1,
      sym_integer_literal,
    ACTIONS(221), 2,
      sym_floating_point_literal,
      sym_string_literal,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(25), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
  [1420] = 8,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(185), 1,
      sym_identifier,
    ACTIONS(223), 1,
      sym_integer_literal,
    ACTIONS(225), 2,
      sym_floating_point_literal,
      sym_string_literal,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(36), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
  [1451] = 9,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(169), 1,
      anon_sym_EQ_EQ,
    ACTIONS(171), 1,
      anon_sym_EQ,
    ACTIONS(227), 1,
      anon_sym_SEMI,
    ACTIONS(165), 2,
      anon_sym_LT,
      anon_sym_GT,
    ACTIONS(167), 2,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
    ACTIONS(173), 2,
      anon_sym_PLUS,
      anon_sym_DASH,
    ACTIONS(175), 3,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [1484] = 8,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(185), 1,
      sym_identifier,
    ACTIONS(229), 1,
      sym_integer_literal,
    ACTIONS(231), 2,
      sym_floating_point_literal,
      sym_string_literal,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(47), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
  [1515] = 9,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(169), 1,
      anon_sym_EQ_EQ,
    ACTIONS(171), 1,
      anon_sym_EQ,
    ACTIONS(233), 1,
      anon_sym_SEMI,
    ACTIONS(165), 2,
      anon_sym_LT,
      anon_sym_GT,
    ACTIONS(167), 2,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
    ACTIONS(173), 2,
      anon_sym_PLUS,
      anon_sym_DASH,
    ACTIONS(175), 3,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [1548] = 9,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(169), 1,
      anon_sym_EQ_EQ,
    ACTIONS(171), 1,
      anon_sym_EQ,
    ACTIONS(235), 1,
      anon_sym_SEMI,
    ACTIONS(165), 2,
      anon_sym_LT,
      anon_sym_GT,
    ACTIONS(167), 2,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
    ACTIONS(173), 2,
      anon_sym_PLUS,
      anon_sym_DASH,
    ACTIONS(175), 3,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [1581] = 8,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(237), 1,
      ts_builtin_sym_end,
    ACTIONS(239), 1,
      anon_sym_func,
    STATE(78), 1,
      sym_variable_declaration,
    STATE(86), 1,
      sym_variable_definition,
    ACTIONS(242), 2,
      anon_sym_var,
      anon_sym_let,
    STATE(48), 6,
      sym__definition,
      sym__declaration,
      sym_function_definition,
      sym_variable_declaration_sentence,
      sym_variable_definition_sentence,
      aux_sym_source_file_repeat1,
  [1612] = 8,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(185), 1,
      sym_identifier,
    ACTIONS(245), 1,
      sym_integer_literal,
    ACTIONS(247), 2,
      sym_floating_point_literal,
      sym_string_literal,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(51), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
  [1643] = 8,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(185), 1,
      sym_identifier,
    ACTIONS(249), 1,
      sym_integer_literal,
    ACTIONS(251), 2,
      sym_floating_point_literal,
      sym_string_literal,
    STATE(9), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(37), 4,
      sym__uncallable_expression,
      sym__expression,
      sym_binary_expresion,
      sym__literal,
  [1674] = 9,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(169), 1,
      anon_sym_EQ_EQ,
    ACTIONS(171), 1,
      anon_sym_EQ,
    ACTIONS(253), 1,
      anon_sym_RPAREN,
    ACTIONS(165), 2,
      anon_sym_LT,
      anon_sym_GT,
    ACTIONS(167), 2,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
    ACTIONS(173), 2,
      anon_sym_PLUS,
      anon_sym_DASH,
    ACTIONS(175), 3,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [1707] = 9,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(15), 1,
      anon_sym_LPAREN,
    ACTIONS(185), 1,
      sym_identifier,
    ACTIONS(255), 1,
      sym_integer_literal,
    STATE(54), 1,
      sym__expression,
    ACTIONS(257), 2,
      sym_floating_point_literal,
      sym_string_literal,
    STATE(34), 3,
      sym__callable_expression,
      sym_function_call,
      sym_variable_reference_expression,
    STATE(53), 3,
      sym__uncallable_expression,
      sym_binary_expresion,
      sym__literal,
  [1740] = 5,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(259), 1,
      anon_sym_RPAREN,
    ACTIONS(95), 3,
      anon_sym_LT,
      anon_sym_GT,
      anon_sym_EQ,
    ACTIONS(93), 8,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
      anon_sym_EQ_EQ,
      anon_sym_PLUS,
      anon_sym_DASH,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [1765] = 8,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(169), 1,
      anon_sym_EQ_EQ,
    ACTIONS(171), 1,
      anon_sym_EQ,
    ACTIONS(165), 2,
      anon_sym_LT,
      anon_sym_GT,
    ACTIONS(167), 2,
      anon_sym_LT_EQ,
      anon_sym_GT_EQ,
    ACTIONS(173), 2,
      anon_sym_PLUS,
      anon_sym_DASH,
    ACTIONS(175), 3,
      anon_sym_STAR,
      anon_sym_SLASH,
      anon_sym_PERCENT,
  [1795] = 6,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(261), 1,
      anon_sym_RPAREN,
    STATE(65), 1,
      aux_sym_parameter_list_repeat1,
    STATE(76), 1,
      sym_variable_declaration,
    ACTIONS(11), 2,
      anon_sym_var,
      anon_sym_let,
  [1815] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(263), 5,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_LBRACE,
      anon_sym_SEMI,
      anon_sym_EQ,
  [1829] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(265), 5,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_LBRACE,
      anon_sym_SEMI,
      anon_sym_EQ,
  [1843] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(267), 4,
      ts_builtin_sym_end,
      anon_sym_func,
      anon_sym_var,
      anon_sym_let,
  [1856] = 5,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    STATE(59), 1,
      aux_sym_parameter_list_repeat1,
    STATE(89), 1,
      sym_variable_declaration,
    ACTIONS(269), 2,
      anon_sym_var,
      anon_sym_let,
  [1873] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(143), 4,
      ts_builtin_sym_end,
      anon_sym_func,
      anon_sym_var,
      anon_sym_let,
  [1886] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(135), 4,
      ts_builtin_sym_end,
      anon_sym_func,
      anon_sym_var,
      anon_sym_let,
  [1899] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(139), 4,
      ts_builtin_sym_end,
      anon_sym_func,
      anon_sym_var,
      anon_sym_let,
  [1912] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(272), 4,
      anon_sym_COMMA,
      anon_sym_RPAREN,
      anon_sym_SEMI,
      anon_sym_EQ,
  [1925] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(147), 4,
      ts_builtin_sym_end,
      anon_sym_func,
      anon_sym_var,
      anon_sym_let,
  [1938] = 5,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    STATE(59), 1,
      aux_sym_parameter_list_repeat1,
    STATE(69), 1,
      sym_variable_declaration,
    ACTIONS(11), 2,
      anon_sym_var,
      anon_sym_let,
  [1955] = 5,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(161), 1,
      anon_sym_COMMA,
    ACTIONS(274), 1,
      anon_sym_RPAREN,
    STATE(67), 1,
      aux_sym_function_call_repeat1,
  [1971] = 5,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(201), 1,
      anon_sym_RPAREN,
    ACTIONS(276), 1,
      anon_sym_COMMA,
    STATE(67), 1,
      aux_sym_function_call_repeat1,
  [1987] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(17), 1,
      anon_sym_LBRACE,
    STATE(18), 1,
      sym_block,
  [2000] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(279), 1,
      anon_sym_COMMA,
    ACTIONS(281), 1,
      anon_sym_RPAREN,
  [2013] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(283), 1,
      anon_sym_LPAREN,
    STATE(80), 1,
      sym_parameter_list,
  [2026] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(285), 2,
      anon_sym_var,
      anon_sym_let,
  [2037] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(287), 1,
      anon_sym_LBRACE,
    STATE(58), 1,
      sym_block,
  [2050] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(17), 1,
      anon_sym_LBRACE,
    STATE(24), 1,
      sym_block,
  [2063] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(17), 1,
      anon_sym_LBRACE,
    STATE(17), 1,
      sym_block,
  [2076] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(17), 1,
      anon_sym_LBRACE,
    STATE(23), 1,
      sym_block,
  [2089] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(279), 1,
      anon_sym_COMMA,
    ACTIONS(289), 1,
      anon_sym_RPAREN,
  [2102] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(291), 1,
      anon_sym_SEMI,
    ACTIONS(293), 1,
      anon_sym_EQ,
  [2115] = 4,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(293), 1,
      anon_sym_EQ,
    ACTIONS(295), 1,
      anon_sym_SEMI,
  [2128] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(297), 1,
      ts_builtin_sym_end,
  [2138] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(299), 1,
      anon_sym_DASH_GT,
  [2148] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(301), 1,
      anon_sym_SEMI,
  [2158] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(303), 1,
      anon_sym_LPAREN,
  [2168] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(305), 1,
      sym_identifier,
  [2178] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(307), 1,
      anon_sym_LPAREN,
  [2188] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(309), 1,
      sym_identifier,
  [2198] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(311), 1,
      anon_sym_SEMI,
  [2208] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(313), 1,
      anon_sym_COLON,
  [2218] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(315), 1,
      anon_sym_DASH_GT,
  [2228] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(279), 1,
      anon_sym_COMMA,
  [2238] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(317), 1,
      anon_sym_DASH_GT,
  [2248] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(319), 1,
      anon_sym_SEMI,
  [2258] = 3,
    ACTIONS(3), 1,
      sym_oneline_comment,
    ACTIONS(5), 1,
      sym_multiline_comment,
    ACTIONS(321), 1,
      anon_sym_DASH_GT,
};

static const uint32_t ts_small_parse_table_map[] = {
  [SMALL_STATE(2)] = 0,
  [SMALL_STATE(3)] = 80,
  [SMALL_STATE(4)] = 160,
  [SMALL_STATE(5)] = 240,
  [SMALL_STATE(6)] = 320,
  [SMALL_STATE(7)] = 400,
  [SMALL_STATE(8)] = 426,
  [SMALL_STATE(9)] = 452,
  [SMALL_STATE(10)] = 480,
  [SMALL_STATE(11)] = 506,
  [SMALL_STATE(12)] = 532,
  [SMALL_STATE(13)] = 558,
  [SMALL_STATE(14)] = 584,
  [SMALL_STATE(15)] = 610,
  [SMALL_STATE(16)] = 636,
  [SMALL_STATE(17)] = 662,
  [SMALL_STATE(18)] = 688,
  [SMALL_STATE(19)] = 714,
  [SMALL_STATE(20)] = 740,
  [SMALL_STATE(21)] = 766,
  [SMALL_STATE(22)] = 792,
  [SMALL_STATE(23)] = 818,
  [SMALL_STATE(24)] = 844,
  [SMALL_STATE(25)] = 870,
  [SMALL_STATE(26)] = 895,
  [SMALL_STATE(27)] = 934,
  [SMALL_STATE(28)] = 967,
  [SMALL_STATE(29)] = 992,
  [SMALL_STATE(30)] = 1019,
  [SMALL_STATE(31)] = 1048,
  [SMALL_STATE(32)] = 1075,
  [SMALL_STATE(33)] = 1102,
  [SMALL_STATE(34)] = 1136,
  [SMALL_STATE(35)] = 1164,
  [SMALL_STATE(36)] = 1198,
  [SMALL_STATE(37)] = 1232,
  [SMALL_STATE(38)] = 1265,
  [SMALL_STATE(39)] = 1296,
  [SMALL_STATE(40)] = 1327,
  [SMALL_STATE(41)] = 1358,
  [SMALL_STATE(42)] = 1389,
  [SMALL_STATE(43)] = 1420,
  [SMALL_STATE(44)] = 1451,
  [SMALL_STATE(45)] = 1484,
  [SMALL_STATE(46)] = 1515,
  [SMALL_STATE(47)] = 1548,
  [SMALL_STATE(48)] = 1581,
  [SMALL_STATE(49)] = 1612,
  [SMALL_STATE(50)] = 1643,
  [SMALL_STATE(51)] = 1674,
  [SMALL_STATE(52)] = 1707,
  [SMALL_STATE(53)] = 1740,
  [SMALL_STATE(54)] = 1765,
  [SMALL_STATE(55)] = 1795,
  [SMALL_STATE(56)] = 1815,
  [SMALL_STATE(57)] = 1829,
  [SMALL_STATE(58)] = 1843,
  [SMALL_STATE(59)] = 1856,
  [SMALL_STATE(60)] = 1873,
  [SMALL_STATE(61)] = 1886,
  [SMALL_STATE(62)] = 1899,
  [SMALL_STATE(63)] = 1912,
  [SMALL_STATE(64)] = 1925,
  [SMALL_STATE(65)] = 1938,
  [SMALL_STATE(66)] = 1955,
  [SMALL_STATE(67)] = 1971,
  [SMALL_STATE(68)] = 1987,
  [SMALL_STATE(69)] = 2000,
  [SMALL_STATE(70)] = 2013,
  [SMALL_STATE(71)] = 2026,
  [SMALL_STATE(72)] = 2037,
  [SMALL_STATE(73)] = 2050,
  [SMALL_STATE(74)] = 2063,
  [SMALL_STATE(75)] = 2076,
  [SMALL_STATE(76)] = 2089,
  [SMALL_STATE(77)] = 2102,
  [SMALL_STATE(78)] = 2115,
  [SMALL_STATE(79)] = 2128,
  [SMALL_STATE(80)] = 2138,
  [SMALL_STATE(81)] = 2148,
  [SMALL_STATE(82)] = 2158,
  [SMALL_STATE(83)] = 2168,
  [SMALL_STATE(84)] = 2178,
  [SMALL_STATE(85)] = 2188,
  [SMALL_STATE(86)] = 2198,
  [SMALL_STATE(87)] = 2208,
  [SMALL_STATE(88)] = 2218,
  [SMALL_STATE(89)] = 2228,
  [SMALL_STATE(90)] = 2238,
  [SMALL_STATE(91)] = 2248,
  [SMALL_STATE(92)] = 2258,
};

static const TSParseActionEntry ts_parse_actions[] = {
  [0] = {.entry = {.count = 0, .reusable = false}},
  [1] = {.entry = {.count = 1, .reusable = false}}, RECOVER(),
  [3] = {.entry = {.count = 1, .reusable = false}}, SHIFT_EXTRA(),
  [5] = {.entry = {.count = 1, .reusable = true}}, SHIFT_EXTRA(),
  [7] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_source_file, 0, 0, 0),
  [9] = {.entry = {.count = 1, .reusable = true}}, SHIFT(83),
  [11] = {.entry = {.count = 1, .reusable = true}}, SHIFT(85),
  [13] = {.entry = {.count = 1, .reusable = false}}, SHIFT(7),
  [15] = {.entry = {.count = 1, .reusable = true}}, SHIFT(52),
  [17] = {.entry = {.count = 1, .reusable = true}}, SHIFT(6),
  [19] = {.entry = {.count = 1, .reusable = true}}, SHIFT(64),
  [21] = {.entry = {.count = 1, .reusable = false}}, SHIFT(44),
  [23] = {.entry = {.count = 1, .reusable = true}}, SHIFT(44),
  [25] = {.entry = {.count = 1, .reusable = false}}, SHIFT(85),
  [27] = {.entry = {.count = 1, .reusable = false}}, SHIFT(35),
  [29] = {.entry = {.count = 1, .reusable = false}}, SHIFT(81),
  [31] = {.entry = {.count = 1, .reusable = false}}, SHIFT(73),
  [33] = {.entry = {.count = 1, .reusable = false}}, SHIFT(82),
  [35] = {.entry = {.count = 1, .reusable = false}}, SHIFT(84),
  [37] = {.entry = {.count = 1, .reusable = false}}, SHIFT(74),
  [39] = {.entry = {.count = 1, .reusable = true}}, SHIFT(60),
  [41] = {.entry = {.count = 1, .reusable = true}}, SHIFT(22),
  [43] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0), SHIFT_REPEAT(7),
  [46] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0), SHIFT_REPEAT(52),
  [49] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0), SHIFT_REPEAT(6),
  [52] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0),
  [54] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0), SHIFT_REPEAT(44),
  [57] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0), SHIFT_REPEAT(44),
  [60] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0), SHIFT_REPEAT(85),
  [63] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0), SHIFT_REPEAT(35),
  [66] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0), SHIFT_REPEAT(81),
  [69] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0), SHIFT_REPEAT(73),
  [72] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0), SHIFT_REPEAT(82),
  [75] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0), SHIFT_REPEAT(84),
  [78] = {.entry = {.count = 2, .reusable = false}}, REDUCE(aux_sym_block_repeat1, 2, 0, 0), SHIFT_REPEAT(74),
  [81] = {.entry = {.count = 1, .reusable = true}}, SHIFT(21),
  [83] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_variable_reference_expression, 1, 0, 0),
  [85] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_variable_reference_expression, 1, 0, 0),
  [87] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__callable_expression, 3, 0, 0),
  [89] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__callable_expression, 3, 0, 0),
  [91] = {.entry = {.count = 1, .reusable = true}}, SHIFT(33),
  [93] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__expression, 1, 0, 0),
  [95] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__expression, 1, 0, 0),
  [97] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_call, 4, 0, 2),
  [99] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_function_call, 4, 0, 2),
  [101] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_call, 3, 0, 2),
  [103] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_function_call, 3, 0, 2),
  [105] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_call, 5, 0, 2),
  [107] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_function_call, 5, 0, 2),
  [109] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_return_sentence, 2, 0, 0),
  [111] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_return_sentence, 2, 0, 0),
  [113] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_break_sentence, 2, 0, 0),
  [115] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_break_sentence, 2, 0, 0),
  [117] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_expression_sentence, 2, 0, 0),
  [119] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_expression_sentence, 2, 0, 0),
  [121] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_return_sentence, 3, 0, 0),
  [123] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_return_sentence, 3, 0, 0),
  [125] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_else_statement, 2, 0, 0),
  [127] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_else_statement, 2, 0, 0),
  [129] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_elif_statement, 5, 0, 0),
  [131] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_elif_statement, 5, 0, 0),
  [133] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_variable_declaration_sentence, 2, 0, 0),
  [135] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_variable_declaration_sentence, 2, 0, 0),
  [137] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_variable_definition_sentence, 2, 0, 0),
  [139] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_variable_definition_sentence, 2, 0, 0),
  [141] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_block, 2, 0, 0),
  [143] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_block, 2, 0, 0),
  [145] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_block, 3, 0, 0),
  [147] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_block, 3, 0, 0),
  [149] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_if_statement, 5, 0, 0),
  [151] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_if_statement, 5, 0, 0),
  [153] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_loop_statement, 2, 0, 0),
  [155] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_loop_statement, 2, 0, 0),
  [157] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_binary_expresion, 3, 0, 0),
  [159] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym_binary_expresion, 3, 0, 0),
  [161] = {.entry = {.count = 1, .reusable = true}}, SHIFT(43),
  [163] = {.entry = {.count = 1, .reusable = true}}, SHIFT(10),
  [165] = {.entry = {.count = 1, .reusable = false}}, SHIFT(38),
  [167] = {.entry = {.count = 1, .reusable = true}}, SHIFT(38),
  [169] = {.entry = {.count = 1, .reusable = true}}, SHIFT(39),
  [171] = {.entry = {.count = 1, .reusable = false}}, SHIFT(39),
  [173] = {.entry = {.count = 1, .reusable = true}}, SHIFT(40),
  [175] = {.entry = {.count = 1, .reusable = true}}, SHIFT(42),
  [177] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym__uncallable_expression, 3, 0, 0),
  [179] = {.entry = {.count = 1, .reusable = false}}, REDUCE(sym__uncallable_expression, 3, 0, 0),
  [181] = {.entry = {.count = 1, .reusable = false}}, SHIFT(57),
  [183] = {.entry = {.count = 1, .reusable = false}}, SHIFT(56),
  [185] = {.entry = {.count = 1, .reusable = true}}, SHIFT(7),
  [187] = {.entry = {.count = 1, .reusable = true}}, SHIFT(11),
  [189] = {.entry = {.count = 1, .reusable = false}}, SHIFT(26),
  [191] = {.entry = {.count = 1, .reusable = true}}, SHIFT(26),
  [193] = {.entry = {.count = 1, .reusable = true}}, SHIFT(8),
  [195] = {.entry = {.count = 1, .reusable = true}}, SHIFT(13),
  [197] = {.entry = {.count = 1, .reusable = false}}, SHIFT(46),
  [199] = {.entry = {.count = 1, .reusable = true}}, SHIFT(46),
  [201] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_function_call_repeat1, 2, 0, 0),
  [203] = {.entry = {.count = 1, .reusable = true}}, SHIFT(68),
  [205] = {.entry = {.count = 1, .reusable = false}}, SHIFT(27),
  [207] = {.entry = {.count = 1, .reusable = true}}, SHIFT(27),
  [209] = {.entry = {.count = 1, .reusable = false}}, SHIFT(30),
  [211] = {.entry = {.count = 1, .reusable = true}}, SHIFT(30),
  [213] = {.entry = {.count = 1, .reusable = false}}, SHIFT(29),
  [215] = {.entry = {.count = 1, .reusable = true}}, SHIFT(29),
  [217] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_source_file, 1, 0, 0),
  [219] = {.entry = {.count = 1, .reusable = false}}, SHIFT(25),
  [221] = {.entry = {.count = 1, .reusable = true}}, SHIFT(25),
  [223] = {.entry = {.count = 1, .reusable = false}}, SHIFT(36),
  [225] = {.entry = {.count = 1, .reusable = true}}, SHIFT(36),
  [227] = {.entry = {.count = 1, .reusable = true}}, SHIFT(15),
  [229] = {.entry = {.count = 1, .reusable = false}}, SHIFT(47),
  [231] = {.entry = {.count = 1, .reusable = true}}, SHIFT(47),
  [233] = {.entry = {.count = 1, .reusable = true}}, SHIFT(16),
  [235] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_variable_definition, 3, 0, 0),
  [237] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0),
  [239] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(83),
  [242] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_source_file_repeat1, 2, 0, 0), SHIFT_REPEAT(85),
  [245] = {.entry = {.count = 1, .reusable = false}}, SHIFT(51),
  [247] = {.entry = {.count = 1, .reusable = true}}, SHIFT(51),
  [249] = {.entry = {.count = 1, .reusable = false}}, SHIFT(37),
  [251] = {.entry = {.count = 1, .reusable = true}}, SHIFT(37),
  [253] = {.entry = {.count = 1, .reusable = true}}, SHIFT(75),
  [255] = {.entry = {.count = 1, .reusable = false}}, SHIFT(53),
  [257] = {.entry = {.count = 1, .reusable = true}}, SHIFT(53),
  [259] = {.entry = {.count = 1, .reusable = true}}, SHIFT(28),
  [261] = {.entry = {.count = 1, .reusable = true}}, SHIFT(88),
  [263] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_builtin_type, 1, 0, 0),
  [265] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_user_type, 1, 0, 0),
  [267] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_function_definition, 6, 0, 3),
  [269] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_parameter_list_repeat1, 2, 0, 0), SHIFT_REPEAT(85),
  [272] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_variable_declaration, 4, 0, 1),
  [274] = {.entry = {.count = 1, .reusable = true}}, SHIFT(12),
  [276] = {.entry = {.count = 2, .reusable = true}}, REDUCE(aux_sym_function_call_repeat1, 2, 0, 0), SHIFT_REPEAT(43),
  [279] = {.entry = {.count = 1, .reusable = true}}, SHIFT(71),
  [281] = {.entry = {.count = 1, .reusable = true}}, SHIFT(90),
  [283] = {.entry = {.count = 1, .reusable = true}}, SHIFT(55),
  [285] = {.entry = {.count = 1, .reusable = true}}, REDUCE(aux_sym_parameter_list_repeat1, 2, 0, 0),
  [287] = {.entry = {.count = 1, .reusable = true}}, SHIFT(3),
  [289] = {.entry = {.count = 1, .reusable = true}}, SHIFT(92),
  [291] = {.entry = {.count = 1, .reusable = true}}, SHIFT(19),
  [293] = {.entry = {.count = 1, .reusable = true}}, SHIFT(45),
  [295] = {.entry = {.count = 1, .reusable = true}}, SHIFT(61),
  [297] = {.entry = {.count = 1, .reusable = true}},  ACCEPT_INPUT(),
  [299] = {.entry = {.count = 1, .reusable = true}}, SHIFT(32),
  [301] = {.entry = {.count = 1, .reusable = true}}, SHIFT(14),
  [303] = {.entry = {.count = 1, .reusable = true}}, SHIFT(49),
  [305] = {.entry = {.count = 1, .reusable = true}}, SHIFT(70),
  [307] = {.entry = {.count = 1, .reusable = true}}, SHIFT(50),
  [309] = {.entry = {.count = 1, .reusable = true}}, SHIFT(87),
  [311] = {.entry = {.count = 1, .reusable = true}}, SHIFT(62),
  [313] = {.entry = {.count = 1, .reusable = true}}, SHIFT(31),
  [315] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_parameter_list, 2, 0, 0),
  [317] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_parameter_list, 4, 0, 0),
  [319] = {.entry = {.count = 1, .reusable = true}}, SHIFT(20),
  [321] = {.entry = {.count = 1, .reusable = true}}, REDUCE(sym_parameter_list, 3, 0, 0),
};

#ifdef __cplusplus
extern "C" {
#endif
#ifdef TREE_SITTER_HIDE_SYMBOLS
#define TS_PUBLIC
#elif defined(_WIN32)
#define TS_PUBLIC __declspec(dllexport)
#else
#define TS_PUBLIC __attribute__((visibility("default")))
#endif

TS_PUBLIC const TSLanguage *tree_sitter_garnet(void) {
  static const TSLanguage language = {
    .abi_version = LANGUAGE_VERSION,
    .symbol_count = SYMBOL_COUNT,
    .alias_count = ALIAS_COUNT,
    .token_count = TOKEN_COUNT,
    .external_token_count = EXTERNAL_TOKEN_COUNT,
    .state_count = STATE_COUNT,
    .large_state_count = LARGE_STATE_COUNT,
    .production_id_count = PRODUCTION_ID_COUNT,
    .supertype_count = SUPERTYPE_COUNT,
    .field_count = FIELD_COUNT,
    .max_alias_sequence_length = MAX_ALIAS_SEQUENCE_LENGTH,
    .parse_table = &ts_parse_table[0][0],
    .small_parse_table = ts_small_parse_table,
    .small_parse_table_map = ts_small_parse_table_map,
    .parse_actions = ts_parse_actions,
    .symbol_names = ts_symbol_names,
    .field_names = ts_field_names,
    .field_map_slices = ts_field_map_slices,
    .field_map_entries = ts_field_map_entries,
    .symbol_metadata = ts_symbol_metadata,
    .public_symbol_map = ts_symbol_map,
    .alias_map = ts_non_terminal_alias_map,
    .alias_sequences = &ts_alias_sequences[0][0],
    .lex_modes = (const void*)ts_lex_modes,
    .lex_fn = ts_lex,
    .primary_state_ids = ts_primary_state_ids,
    .name = "garnet",
    .max_reserved_word_set_size = 0,
    .metadata = {
      .major_version = 0,
      .minor_version = 1,
      .patch_version = 0,
    },
  };
  return &language;
}
#ifdef __cplusplus
}
#endif
